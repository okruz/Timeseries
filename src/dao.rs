...
impl Dao {
    ...
    pub fn get_time_series(
        ...
        let mut time_series = stmt.query_row(params![id], |row| {
            Ok(TimeSeries {
                id: row.get(0)?,
                name: row.get(1)?,
                unit: row.get(2)?,
        ...
        if with_entries {
            let date_string = start_date
                ...
                .unwrap_or("".to_string());
            let mut entries_stmt = self.conn.prepare(
                "SELECT date, value FROM time_series_entry WHERE time_series_id = (?1) AND date >= (?2) ORDER BY date",
            )?;
            let entry_iter = entries_stmt.query_map(params![id, date_string], |row| {
    ...
    pub fn get_plot(
        ...
        let mut plot = stmt.query_row(params![id], |row| {
            Ok(Plot {
                id: row.get(0)?,
                name: row.get(1)?,
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT id FROM time_series WHERE plot_id = (?1)")?;
        let id_iter = stmt.query_map(params![id], |row| Ok(row.get::<_, i64>(0)?))?;

        for id in id_iter {
            plot.time_series
                .push(self.get_time_series(id?, with_entries, start_date)?);
        }
        Ok(plot)
    }
